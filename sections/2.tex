...
\begin{theorem}
    $f \in C[a, b] \Rightarrow S_{\tau, \theta}(f) \underset{|\tau| \to 0} {\to} \int_a^b f(x) dx$, \\ \\ т.е. $\forall \varepsilon > 0 \,\, \exists \delta > 0 : \forall \tau, \theta : | \tau | < \delta \Rightarrow \abs{ \int_a^b f dx - S_{\tau, \theta}(f)} < \varepsilon$
\end{theorem}
