...
\begin{enumerate}
    ...
    \item $f : [\alpha, \be] \to \R_+ = [0, +\infty)$, $0 \leqslant \al < \be \leqslant 2 \pi $, $\{ (r, \phi) \mid \phi \in [\al, \be], r \in [0, f(\phi)] \} = \widetilde{P}_f$ --- криволинейный сектор, $f \in C[\al, \be]$
...
Плохая ситуация, т.к. нельзя провести две вертикальные прямые и посчитать как площадь криволинейной трапеции, но можно разбить на много параметрически заданных криволинейных трапеций:
...
\quad 
$T \subset \{ (x, y, z) \mid x \in [a, b]\}$

$T(x) = \{ (y, z) \mid (x, y, z) \in T \}$ --- сечение T
...
"блинчик" $ = \{ (x, y, z) \mid x \in [x_{j - 1}, x_j], (y, z) \in T(x)\}$ --- почти цилиндр
...
$x_*(\delta), x^*(\delta)$ --- абсциссы минимального и максимального сечений. 
...
$T = \{ (x, y, z) \mid \sqrt{y ^ 2 + z ^ 2} \leqslant f(x) \}, f \in C[a, b], f \geqslant 0$
...
\begin{motivation}
    ...
    Подход физиков --- найти скорость : $\sqrt{x'(t) ^ 2 + y'(t) ^ 2}$ и взять интеграл $\int_a ^ b \sqrt{x'(t) ^ 2 + y'(t) ^ 2} dt$ --- длина кривой
\end{motivation}
